...
def main(args):
        ...
        try:
            with open(fitfile_nom, "r") as j_nom:
                ...
                if args.bkg == "func":
                    ...
                    counts_nom, _ = np.histogram(valid_masses, bins=v_bins)
    ...
    mode_str = "NO FIT (FROZEN BKG)"
    print(f"Generating {args.toys} {args.method} toys for {args.trigger} | Mode: {mode_str} | Bkg: {args.bkg}")
    ...
    out_file = os.path.join("results", f"global_stat_{args.trigger}_{args.method}_{args.jobid}_NOFIT_{bkg_tag}.npy")
