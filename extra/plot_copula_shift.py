    ...
    for channel in CHANNELS:
        ...
        if len(valid_ranks) > 0:
            ...
            if np.sum(sig_hist_xy) > 0:
                ...
                # Calculate local acceptance fraction at the injected mass
                b_xy_local = np.interp(args.mass, centers_xy, B_xy)
                b_jj_local = np.interp(args.mass, centers_jj, B_jj)
                acceptance_ratio = b_xy_local / b_jj_local if b_jj_local > 0 else 0

                # Scale by BOTH the trigger overlap and the sub-channel acceptance
                target_events = args.events * eff * acceptance_ratio
                sig_hist_xy = sig_hist_xy * (target_events / np.sum(sig_hist_xy))
