...
def main():
    parser = argparse.ArgumentParser(description="Plot Full Pearson Matrix: Empirical vs Parametric Copulas")
    ...
    for i in range(n_cols):
        for j in range(i + 1, n_cols):
            ...
            corr_raw[i, j] = corr_raw[j, i] = safe_pearson(m_i_all[raw_mask], m_j_all[raw_mask])
            ...
            corr_boot[i, j] = corr_boot[j, i] = safe_pearson(m_i_boot[boot_mask], m_j_boot[boot_mask])
    ...
    matrices = [
        (corr_raw, "Raw Data Pearson $r$\n(In-Window)", "raw"),
        (corr_boot, f"Empirical Bootstrap Pearson $r$\n($10^7$ Event Toys)", "bootstrap"),
        (corr_emp, f"Empirical Copula Pearson $r$\n($10^7$ Event Toys)", "empirical_copula"),
        (corr_gauss, f"Gaussian Copula Pearson $r$\n($10^7$ Event Toys)", "gaussian_copula"),
        (corr_student, f"Student-t Copula Pearson $r$\n($10^7$ Event Toys)", "student_copula"),
        (corr_indep, f"Independent Poisson Pearson $r$\n(Uncorrelated Control)", "independent")
    ]
    ...
    plt.suptitle(f"Global Correlation Preservation Comparison - Trigger {args.trigger.upper()}", fontsize=24, fontweight='bold', y=1.02)
    plt.tight_layout()
    
    out_path = os.path.join(base_dir, "plots", f"full_pearson_matrix_comparison_6panel_{args.trigger}.png")
    fig.savefig(out_path, dpi=300, bbox_inches='tight', pad_inches=0.05)
    plt.close(fig)
    print(f"Successfully saved 6-panel Pearson correlation matrix plot to: {out_path}")
    ...
    for mat, title, suffix in matrices:
        ...
        # Add trigger name to the individual plot title for clarity
        ...
        out_path_indiv = os.path.join(base_dir, "plots", f"pearson_matrix_{suffix}_{args.trigger}.png")
